...
class ItemAdapterRP(private val context: Context? = null): RecyclerView.Adapter<ItemAdapterRP.ItemViewHolder>(),
    Filterable {
    private var listItem: ArrayList<ItemXXX> = ArrayList()
    private var listItemFull: ArrayList<ItemXXX> = ArrayList(listItem)
...
//            context.startActivity(intent)
    ...
    fun setListItem(listItem: List<ItemXXX>) {
    ...
    fun addItem(item: ItemXXX) {
        listItem.add(item)
        notifyItemInserted(listItem.size - 1)
    }

    fun updateItem(position: Int, item: ItemXXX) {
    ...
    private val myFilter: Filter = object : Filter() {
        override fun performFiltering(constraint: CharSequence?): FilterResults? {
            val filteredList: ArrayList<ItemXXX> = ArrayList()
        ...
        override fun publishResults(
            ...
            listItem.clear()
            listItem.addAll(results.values as List<ItemXXX>)
